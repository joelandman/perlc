...
## Test Suite (49 files; 36/49 pass `tests/harness.sh`'s real-Perl-output comparison as of 2026-07-09)

**The "69/69 passing" claim below is stale and refers to `make test`, which only runs 4 assertion-based files — it does NOT mean all listed tests match real Perl output.** `tests/harness.sh` (compiles + runs each test under both `perlc` and real `perl`, diffs the output) is the actual correctness gate; as of the last full run, 13 of these exact files listed below (`arith.pl`, `features.pl`, `fileio.pl`, `fileops.pl`, `mbs.pl`, `regex_named.pl`, `tier1.pl`, `tier2.pl`, `advanced.pl`, `eval_exception.pl`, `eval_string.pl`, `wantarray_extended.pl`, `test_do_filename.pl`) diverge from real Perl — see `TESTS.md`'s Defect Registry for root causes (most are real bugs, a couple are expected stub behavior or missing-`use warnings`-diagnostics artifacts). Run `./tests/harness.sh` yourself before trusting either number.
