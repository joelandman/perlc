...
| `src/mini-gmp.h/c` | mini-gmp: BSD-licensed GMP subset for `Math::BigInt` (zero external dependency) |
...
## Test Suite (209 files on disk; 201/205 compared by `tests/harness.sh` pass as of 2026-08-23)
...
### Not Yet Implemented
- Prototypes, typeglobs, signals, unicode handling (basic UTF-8 support added for chr/ord/length/substr)
- `use overload` is now **implemented** (D97, 2026-08-19); `Math::BigInt` is now **implemented** using mini-gmp (D97, 2026-08-19, zero external dependency)
...
**Last Updated**: 2026-08-19. Recent additions (this commit):
1. **D96: FLAT_ARRAY row compound-assign crash fix**. The row AV cache (`ra`) is null for flat rows; compound-assign writes (`$ref->[$i][$k] op= expr`) crashed or corrupted data. Fixed via `perl_flat_row_op_assign` runtime helper that re-reads the row PV's tag at runtime. Fixes `nb.pl`/`nbody.pl` segfault.
2. **D97: `use overload` (operator overloading)**. Full parser+codegen+runtime support. `use overload '+' => \&add, ...` parsed into `NK::OverloadStmt`; overload dispatch table with `@ISA` traversal in `perl_add`/`perl_sub`/`perl_mul`/`perl_div`/`perl_mod`/`perl_pow`/`perl_negate`/`perl_num_eq`/etc.; `""` overload in `perl_to_string_dup`; `canEmitF64` excludes blessed-object variables.
3. **D97: `Math::BigInt` built-in using mini-gmp** (zero external dependency). `PERL_BIGINT` tag (17) with `mpz_t*` payload; `new`/`bmul`/`badd`/`bsub`/`bcmp`/`numify`/`bstr`/`copy`/`bneg`/`config` methods; arithmetic overloads (`+`/`-`/`*`/`/`/`<=>`/`""`/`neg`) with floor division and swapped-operand handling; in-place mutator chaining via codegen intercept; `config->{lib}` returns `"Math::BigInt::GMP"`; `--mini-gmp` CLI flag. `pidigits.pl` produces correct pi digits.
