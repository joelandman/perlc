...
- **Threads**: `use threads`; `threads->create(sub{...}, @args)`, `$thr->join()`, `$thr->detach()`, `$thr->tid()`, `threads->self()`, `threads->list()`, `threads->yield()`; thread-local freelist/eval-stack/captures via `__thread`; `PERL_THREAD` tag (11); closure capture deep-copied per thread for isolation (non-shared captured vars are independent copies); shared captured scalars use the tagged-cell layout so both threads see the same cell (reads inside the sub go through `perl_atomic_load` automatically)
- **threads::shared**: `use threads::shared`; `my $x : shared` / `my @arr : shared` / `my %hash : shared`; tagged-cell layout (a shared scalar is a `PerlValue*` with `PV_FLAG_SHARED`, no wrapper); `SharedMutex` is lazy-installed on first `lock()` / `cond_wait()` call and kept in a process-wide side-table; reads/writes go through `perl_atomic_load` / `perl_atomic_store`, RMW (`$x++`, `$x = $x + 1`, `$x += N`) goes through `perl_atomic_inc` / `perl_atomic_add`; `lock($x)` / `lock(@arr)` / `lock(%hash)` with auto-unlock at block exit; per-thread re-entry on the SharedMutex makes `lock($x); $x = $x + 1` safe; `cond_wait($x)` / `cond_signal($x)` / `cond_broadcast($x)`; see `THREADS_SHARED_ATOMIC.md` for the full memory model and cost table
...
- Thread atomicity: `threads_atomic.pl` (Phase-1 contract for the new model — visibility-without-lock, RMW atomicity, cond_wait/signal, cond_broadcast, lock auto-release, plain-var isolation; see `THREADS_SHARED_ATOMIC.md`)
