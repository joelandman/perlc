...
1. **`use overload`**
2. **String `eval EXPR`** (deliberate post-JIT scope cut; `eval { BLOCK }` works)
3. **Regex `/x`** extended modifier
4. **`syscall()`** builtin
5. prototypes, typeglobs, full signals beyond `%SIG` handlers
...
## Test Suite (203 files on disk; 131/138 compared by `tests/harness.sh` pass as of 2026-08-11)
