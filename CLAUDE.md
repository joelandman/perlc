...
- **`do FILE`** — **NOT actually functional** (D24): the runtime stub only checks the file is readable and returns fake success; it never parses or executes the target file post-JIT-removal. 4/7 checks in `tests/test_do_filename.pl` fail because of this. Needs re-implementation, not just a doc fix.
