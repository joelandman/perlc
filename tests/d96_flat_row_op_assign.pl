#!/usr/bin/env perl
# D96: compound-assign (+=,-=) on 2D FLAT_ARRAY row elements inside a sub with
# DerefAV-cached @_ array-ref param.  Previously crashed (segfault from
# perl_array_get_ref(null,...)) or silently corrupted data.  Tests the nb.pl/
# nbody.pl pattern: advance() mutates velocities [3,4,5] in a nested loop, then
# energy() reads them back.  All results verified byte-for-byte against real Perl.
use strict;
...
# section 1: energy before
printf "%.9f\n", energy( 5, \@bodies );

# section 2: advance 1 iteration, check energy
