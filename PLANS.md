...
2. **Add TSan to `make test` for threaded tests.** Currently TSan verification is manual (`./perlc tests/threads.pl -o /tmp/t_tsan && valgrind --tool=drd`). Promote TSan-instrumented builds to an automatic gate: `make test-tsan` builds `perlc_tsan` and runs `threads.pl`, `threads_atomic.pl`, and `destroy.pl`. This catches data races on shared scalars, the shared-mutex side-table, and closure-capture arrays that the current assertion-based tests cannot detect.
